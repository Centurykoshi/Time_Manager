...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    // If goal progress changed, record an XP adjustment item so the dashboard stays in sync.
    try {
      if (existing && goal && body.currentValue !== undefined) {
        ...
        if (delta !== 0) {
          ...
          const actionLabel = delta > 0 ? "Goal check-in" : "Goal rollback";
          const absDelta = Math.abs(delta);

          await prisma.todoItem.create({
            data: {
              title: `${actionLabel}: ${goal.title}`,
              description: `${delta > 0 ? "Logged" : "Reversed"} ${absDelta} ${goal.unit} for ${goal.title}. This keeps the goal progress and XP total aligned.`,
              isDone: true,
              difficulty: "MEDIUM",
              xpEarned: totalXp,
