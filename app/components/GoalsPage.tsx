...
export function GoalsPage() {
  ...
  const applyGoalData = (groupsData: GoalGroup[], goalsData: GoalRecord[]) => {
    setGroups(groupsData);
    setGoals(goalsData);
    setIsLoading(false);
  };

  const writeGoalData = (groupsData: GoalGroup[], goalsData: GoalRecord[]) => {
    applyGoalData(groupsData, goalsData);
    setGoalCache({ groups: groupsData, goals: goalsData });
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeGoalCache((nextData) => {
      ...
      applyGoalData(nextData.groups, nextData.goals);
    });
    ...
    const loadAll = async () => {
      try {
        ...
        if (cachedData) {
          if (active) {
            applyGoalData(cachedData.groups, cachedData.goals);
          }
          return;
        }
  ...
  const createGoal = async (group: GoalGroup, title: string, target: number, description: string, unit: string, difficulty: Difficulty) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const nextGoals = [j.goal, ...goals];
        writeGoalData(groups, nextGoals);
  ...
  const handleUpdateProgress = async (id: string, newValue: number) => {
    try {
      ...
      if (response.ok) {
        const payload = (await response.json()) as { goal: GoalRecord };
        const nextGoals = goals.map((x) => (x.id === id ? payload.goal : x));
        writeGoalData(groups, nextGoals);
        window.dispatchEvent(new Event("dashboard:changed"));
