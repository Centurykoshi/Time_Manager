...
export function TimerPanel() {
  ...
  const [soundType, setSoundType] = useState("water");
  const [soundUrl, setSoundUrl] = useState("/media/water.mp3");
  ...
  const lastAdjustmentSoundAtRef = useRef(0);
  ...
  const restart = async () => {
    ...
    // Reset UI/audio immediately for snappy restart, save elapsed session in background.
    stopTick();
    stopAudio();
    timerEndAtRef.current = null;
    setActiveDurationSec(durationSec);
    ...
    if (!savedRunRef.current && elapsedSeconds > 0) {
      savedRunRef.current = true;
      void saveSession(elapsedSeconds)
        .catch(() => {
          // Keep restart instant even if save fails.
        })
        .finally(() => {
          savedRunRef.current = false;
        });
  ...
  const playAdjustmentSound = async (frequency: number = 800) => {
    ...
    if (nowMs - lastAdjustmentSoundAtRef.current < 80) return;
    lastAdjustmentSoundAtRef.current = nowMs;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 min-h-0 flex-col items-center justify-start gap-5 pt-8 text-center">
        <motion.div
          initial={{ scale: 0.95 }}
          animate={{ scale: 1 }}
