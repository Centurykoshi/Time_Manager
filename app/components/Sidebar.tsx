...
import { Home, ListTodo, Palette, Target, Trophy } from "lucide-react";
...
export function Sidebar({ activePage, onPageChange, todoCount, goalCount, xpCount }: SidebarProps) {
  ...
  return (
    <SidebarShell collapsible="icon" className="border-r border-sidebar-border/60 bg-sidebar/95 backdrop-blur-xl">
      ...
      <SidebarFooter className="border-t border-sidebar-border/60 px-2 py-3 group-data-[collapsible=icon]:px-1.5">
        <SidebarGroup>
          <SidebarGroupLabel className="px-2 py-1.5 text-xs font-semibold text-sidebar-foreground/50 group-data-[collapsible=icon]:hidden">
            Customization
          </SidebarGroupLabel>
          <SidebarGroupContent>
            <SidebarMenu>
                ...
                <Tooltip delayDuration={200}>
                  <TooltipTrigger asChild>
                    ...
                    >
                      ...
                      <span className="relative z-10 flex w-full items-center gap-3 group-data-[collapsible=icon]:w-auto group-data-[collapsible=icon]:justify-center">
                        <Palette className={cn("h-4 w-4 shrink-0", activePage === "themes" ? "text-primary-foreground" : "text-sidebar-foreground/70")} />
                        <span className="truncate text-left group-data-[collapsible=icon]:hidden">Customization</span>
                        <span
                          className={cn(
                            "ml-auto shrink-0 rounded-full px-2 py-0.5 text-[11px] font-semibold tabular-nums group-data-[collapsible=icon]:hidden",
                            activePage === "themes"
                  ...
                  <TooltipContent side="right">Customization</TooltipContent>
                </Tooltip>
              </SidebarMenuItem>
            </SidebarMenu>
          </SidebarGroupContent>
