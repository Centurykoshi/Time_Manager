...
function toGoalRecord(goal: Goal): GoalRecord {
  return {
    ...goal,
...
const cadenceLabel: Record<GoalRecord["cadence"], string> = {
  WEEKLY: "Week",
  MONTHLY: "Month",
  YEARLY: "Year",
  ALL_TIME: "All time",
...
export function GoalPanel() {
  ...
  const applyGoals = (nextGoals: GoalRecord[]) => {
    setGoals(nextGoals);
    setLoading(false);
  };

  const writeGoals = (nextGoals: GoalRecord[]) => {
    applyGoals(nextGoals);
    const currentCache = getGoalCache();
    setGoalCache({
      groups: currentCache?.groups ?? [],
      goals: nextGoals,
    });
  };
  ...
  useEffect(() => {
    let active = true;
    const unsubscribe = subscribeGoalCache((nextData) => {
      if (!active || !nextData) return;
      applyGoals(nextData.goals);
    });
    ...
    const run = async () => {
      try {
        setLoading(true);
        setError(null);
        const cachedData = getGoalCache();
        if (cachedData) {
          if (active) applyGoals(cachedData.goals);
          return;
        }

        const nextData = await loadRemoteGoals();
        if (active) applyGoals(nextData.goals);
      } catch (loadError) {
  ...
  const createGoal = async () => {
    ...
    try {
      ...
      writeGoals([toGoalRecord(payload.goal), ...goals]);
      setTitle("");
      setDescription("");
      setTargetValue("20");
      setUnit("sessions");
  ...
  const updateGoal = async (id: string, patch: Partial<Pick<Goal, "currentValue" | "isArchived">>) => {
    ...
    try {
      ...
      const payload = (await response.json()) as { goal: Goal };
      if (payload.goal) {
        writeGoals(goals.map((goal) => (goal.id === id ? toGoalRecord(payload.goal) : goal)));
      }
    } catch {
      writeGoals(goals.map((goal) => (goal.id === id ? target : goal)));
